...
export default async function PricingPage({
  ...
  if (session?.user) {
    const ownerMembership = await prisma.storeMember.findFirst({
      ...
      include: {
        store: { include: { subscription: { include: { plan: true } } } },
      },
    })
    ...
    <div className="flex flex-1 flex-col">
      <div className="@container/main flex flex-1 flex-col gap-2">
        <div className="flex flex-col gap-8 py-4 px-4 md:gap-10 md:py-6 lg:px-6">
          ...
          {/* Redirect feedback alerts */}
          {params.subscribed && (
            <Alert>
              <SparklesIcon className="size-4" />
              <AlertTitle>Subscription Activated!</AlertTitle>
              <AlertDescription>
                Welcome aboard. Your plan is now active and your store is ready to use.
              </AlertDescription>
            </Alert>
          )}
          ...
          {currentStatus === "EXPIRED" && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Your subscription has expired. Please renew below to continue.
              </AlertDescription>
            </Alert>
          )}
          ...
          <p className="text-center text-xs text-muted-foreground pb-4">
            All prices are in IDR and exclusive of applicable taxes.
